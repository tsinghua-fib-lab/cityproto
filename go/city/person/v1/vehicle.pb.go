...
type LC struct {
	...
	ShadowLaneId int32 `protobuf:"varint,1,opt,name=shadow_lane_id,json=shadowLaneId,proto3" json:"shadow_lane_id,omitempty"`
	// 投影到影子车道的坐标
	// s coordinate projected to shadow lane
	ShadowS float64 `protobuf:"fixed64,2,opt,name=shadow_s,json=shadowS,proto3" json:"shadow_s,omitempty"`
	...
	CompletedRatio float64 `protobuf:"fixed64,4,opt,name=completed_ratio,json=completedRatio,proto3" json:"completed_ratio,omitempty"`
}
...
type VehicleAction struct {
	...
	LcTargetId *int32 `protobuf:"varint,2,opt,name=lc_target_id,json=lcTargetId,proto3,oneof" json:"lc_target_id,omitempty"`
	// 变道过程的转向角度
...
type VehicleRuntime struct {
	...
	Base *PersonMotion `protobuf:"bytes,1,opt,name=base,proto3" json:"base,omitempty"`
	// 变道信息
	// lane change information
	Lc *LC `protobuf:"bytes,4,opt,name=lc,proto3,oneof" json:"lc,omitempty"`
	// 本轮车辆行为（获取车辆环境信息时不返回）
	// vehicle action in the step (not returned when getting vehicle environment information)
	Action *VehicleAction `protobuf:"bytes,5,opt,name=action,proto3,oneof" json:"action,omitempty"`
	// 走过的里程
	// running distance
	RunningDistance float64 `protobuf:"fixed64,6,opt,name=running_distance,json=runningDistance,proto3" json:"running_distance,omitempty"`
	// 走错路次数
	// number of going astray
	NumGoingAstray int32 `protobuf:"varint,7,opt,name=num_going_astray,json=numGoingAstray,proto3" json:"num_going_astray,omitempty"`
	// 出发时刻
	// departure time
	DepartureTime float64 `protobuf:"fixed64,8,opt,name=departure_time,json=departureTime,proto3" json:"departure_time,omitempty"`
	// 预计到达时刻（导航返回的eta+出发时刻）
	// estimated arrival time (eta returned by routing + departure time)
	Eta float64 `protobuf:"fixed64,9,opt,name=eta,proto3" json:"eta,omitempty"`
	// 自由流下的预计到达时刻
	// estimated arrival time under free flow
	EtaFreeFlow float64 `protobuf:"fixed64,10,opt,name=eta_free_flow,json=etaFreeFlow,proto3" json:"eta_free_flow,omitempty"`
}
...
type ObservedVehicle struct {
	...
	// 相对距离
	// relative distance
	RelativeDistance float64 `protobuf:"fixed64,3,opt,name=relative_distance,json=relativeDistance,proto3" json:"relative_distance,omitempty"`
	// 关系枚举
	// relation enumeration
	Relation VehicleRelation `protobuf:"varint,4,opt,name=relation,proto3,enum=city.person.v1.VehicleRelation" json:"relation,omitempty"`
}
...
type ObservedLane struct {
	...
	// 是否限行
	// whether restricted
	Restriction bool `protobuf:"varint,2,opt,name=restriction,proto3" json:"restriction,omitempty"`
	// 交通灯状态
	// traffic light state
	LightState LightState `protobuf:"varint,3,opt,name=light_state,json=lightState,proto3,enum=city.person.v1.LightState" json:"light_state,omitempty"`
	// 交通灯剩余时间
	// remaining time of traffic light
	LightRemainingTime float64 `protobuf:"fixed64,4,opt,name=light_remaining_time,json=lightRemainingTime,proto3" json:"light_remaining_time,omitempty"`
}
...
type VehicleEnv struct {
	...
	// 当前的车辆运行时信息
	// current vehicle runtime information
	Runtime *VehicleRuntime `protobuf:"bytes,2,opt,name=runtime,proto3" json:"runtime,omitempty"`
	// 当前的路径规划结果
	// journey: current routing result
	Journey *v2.Journey `protobuf:"bytes,3,opt,name=journey,proto3" json:"journey,omitempty"`
	// 观测到的车辆
	// observed vehicles
	ObservedVehicles []*ObservedVehicle `protobuf:"bytes,4,rep,name=observed_vehicles,json=observedVehicles,proto3" json:"observed_vehicles,omitempty"`
	// 观测到的车道状态
	// observed lane states
	ObservedLanes []*ObservedLane `protobuf:"bytes,5,rep,name=observed_lanes,json=observedLanes,proto3" json:"observed_lanes,omitempty"`
}
