// @generated by protoc-gen-connect-es v1.5.0
// @generated from file city/map/v2/road_service.proto (package city.map.v2, syntax proto3)
/* eslint-disable */
// @ts-nocheck

import { GetEventsRequest, GetEventsResponse, GetRoadGlobalStatisticsRequest, GetRoadGlobalStatisticsResponse, GetRoadRequest, GetRoadResponse, GetRuinInfoRequest, GetRuinInfoResponse } from "./road_service_pb.js";
import { MethodKind } from "@bufbuild/protobuf";

/**
 * @generated from service city.map.v2.RoadService
 */
export declare const RoadService: {
  readonly typeName: "city.map.v2.RoadService",
  readonly methods: {
    /**
     * 查询道路信息
     * Get road information
     *
     * @generated from rpc city.map.v2.RoadService.GetRoad
     */
    readonly getRoad: {
      readonly name: "GetRoad",
      readonly I: typeof GetRoadRequest,
      readonly O: typeof GetRoadResponse,
      readonly kind: MethodKind.Unary,
    },
    /**
     * 获取全局统计信息
     * Get global statistics
     *
     * @generated from rpc city.map.v2.RoadService.GetRoadGlobalStatistics
     */
    readonly getRoadGlobalStatistics: {
      readonly name: "GetRoadGlobalStatistics",
      readonly I: typeof GetRoadGlobalStatisticsRequest,
      readonly O: typeof GetRoadGlobalStatisticsResponse,
      readonly kind: MethodKind.Unary,
    },
    /**
     * @generated from rpc city.map.v2.RoadService.GetRuinInfo
     */
    readonly getRuinInfo: {
      readonly name: "GetRuinInfo",
      readonly I: typeof GetRuinInfoRequest,
      readonly O: typeof GetRuinInfoResponse,
      readonly kind: MethodKind.Unary,
    },
    /**
     * @generated from rpc city.map.v2.RoadService.GetEvents
     */
    readonly getEvents: {
      readonly name: "GetEvents",
      readonly I: typeof GetEventsRequest,
      readonly O: typeof GetEventsResponse,
      readonly kind: MethodKind.Unary,
    },
  }
};

